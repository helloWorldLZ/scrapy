...
class PhoneSuningSpider(scrapy.Spider):
    name = 'phone_suning'
    ...
    def start_requests(self):
        ...
        function main(splash, args)
            ...
            splash:select('#bottomPage'):setAttribute('value', args.pageNum)
            splash:select('#bottom_pager .ensure'):click()
            splash:wait(args.wait)
        ...
        argsDict = {
            'url': r'https://search.suning.com/手机/',
            'wait': 5,
            'pageNum': 2,
            'lua_source': script,
        }
    ...
    def parse(self, response):
        ...
        urls = response.css('#product-list li.item-wrap .res-img .img-block a::attr(href)').getall()
        # urls = response.css('#bottom_pager a.cur::text').getall()
